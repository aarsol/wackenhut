...
class HRLoanWaiveOffWiz(models.TransientModel):
    _name = 'hr.loan.waiveoff.wiz'
    _description = 'Write Off Wizard'
    ...
    date = fields.Date('Write Off Date', default=fields.Date.today())
    ...
    def action_loan_waiveoff(self):
        for rec in self:
            if rec.loan_id.wf_move_id:
                raise UserError(_('This Loan is already Waived Off.'))
            if rec.loan_id.lsp_move_id:
            ...
            name = _('Mr. %s Loan Waived Off') % (rec.loan_id.employee_id.name)
            line_ids = []
            move = {
                'narration': name,
                'date': rec.date,
            ...
            if move_id:
                move_id.action_post()
                rec.loan_id.wf_move_id = move_id and move_id.id or False
    ...
    def _compute_show_message_text(self):
        for rec in self:
            if rec.loan_id and rec.loan_id.wf_move_id:
