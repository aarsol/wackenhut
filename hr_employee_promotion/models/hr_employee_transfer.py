...
class HREmployeeTransfer(models.Model):
    ...
    transfer_date = fields.Date('Transfer Date', default=fields.Date.today(), tracking=True)
    transfer_type = fields.Many2one('hr.employee.transfer.type', string="Transfer Type", tracking=True)
        ...
        for rec in self:
            if rec.is_create_new_contract:
                ...
                if contract_id:
                    ...
                    new_contract_date = rec.transfer_date + datetime.timedelta(days=1)
            ...
            rec.employee_id.department_id = rec.new_department_id and rec.new_department_id.id or False
            rec.employee_id.job_id = rec.new_job_id and rec.new_job_id.id or False
            rec.state = 'approve'
    ...
    def create(self, values):
        res = super(HREmployeeTransfer, self).create(values)
        if not res.name:
            res.name = self.env['ir.sequence'].next_by_code('hr.employee.transfer')
        return res
