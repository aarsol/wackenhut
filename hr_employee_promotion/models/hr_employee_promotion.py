...
class HREmployeePromotion(models.Model):
    ...
    promotion_date = fields.Date('Promotion Date', default=fields.Date.today(), tracking=True)
    promotion_type = fields.Many2one('hr.employee.promotion.type', string="Promotion Type", tracking=True)
        ...
        for rec in self:
            if rec.is_create_new_contract:
                ...
                if contract_id:
                    ...
                    new_contract_date = rec.promotion_date + datetime.timedelta(days=1)
    ...
    def create(self, values):
        res = super(HREmployeePromotion, self).create(values)
        if not res.name:
            res.name = self.env['ir.sequence'].next_by_code('hr.employee.promotion')
        return res
