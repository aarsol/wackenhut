...
class HREmployeeDemotionWizard(models.TransientModel):
    _name = 'hr.employee.demotion.wizard'
    _description = 'Employee Demotion Wizard'
    ...
    promotion_type = fields.Many2one('hr.employee.promotion.type', string="Demotion Type")
    ...
    def action_demote_employee(self):
        for rec in self:
            res = {
                ...
                'demotion_date': rec.demotion_date,
            ...
            demotion_rec = self.env['hr.employee.demotion'].create(res)

            if demotion_rec:
                demotion_list = demotion_rec.mapped('id')
                form_view = self.env.ref('hr_employee_promotion.view_hr_employee_demotion_form')
                tree_view = self.env.ref('hr_employee_promotion.view_hr_employee_demotion_tree')
                return {
                    'domain': [('id', 'in', demotion_list)],
                    'name': _('Employee Demotion'),
                    'view_type': 'form',
                    'view_mode': 'tree,form',
